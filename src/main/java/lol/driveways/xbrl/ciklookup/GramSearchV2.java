...
public class GramSearchV2 implements Search {

    private static final Logger log = Logger.getLogger(GramSearchV2.class);
...
//                .map(loaderThing::getGramCache).filter(Optional::isPresent).map(Optional::get);
    ...
    final Integer maxQueue = 100;

    private Stream<CIKScore> topScores(final Stream<CIKScore> cikScores) {
        final PriorityQueue<CIKScore> queue = new PriorityQueue<>();
        cikScores.forEach((score) -> {
            if (queue.size() >= maxQueue) {
                final CIKScore minScore = queue.poll();
                if (score.getScore() > minScore.getScore()) {
                    queue.offer(score);
                } else {
                    queue.offer(minScore);
                }
            } else {
                queue.offer(score);
            }
